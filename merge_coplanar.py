...
def merge_component(mesh, axis_id, face_indices, cid, debug=True):
    ...
    #
    # Special case for +Z (axis_id == 4): a parentless loop with no children
    # is an OPEN HOLE in the mesh surface (intentional through-holes).
    # These must not be triangulated — only fill +Z outers that contain at
    # least one hole loop inside them (solid regions with recesses).
    PLUS_Z = 4
    ...
    for lp_o, p2d_o, hole_lps, hole_2ds in clusters:
        ...
        # On +Z faces, hole rings are open holes — do NOT triangulate them
        if axis_id == PLUS_Z:
            continue
...
def pack_faces(faces):
    ...
    7x faster than frozenset-based lookups."""
    sf = np.sort(faces, axis=1).astype(np.int64)
    n  = int(sf.max()) + 1
    return sf[:,0]*n*n + sf[:,1]*n + sf[:,2], n


def build_lookup(faces):
    """Build {packed_key: row_index} for fast face resolution."""
    keys, _ = pack_faces(faces)
    return dict(zip(keys.tolist(), range(len(faces))))


def apply_batch(mesh, pending, lookup=None):
    """Apply a list of (face_keys, new_faces) replacements in one shot."""
    if lookup is None:
        lookup = build_lookup(mesh.faces)
    keep = np.ones(len(mesh.faces), dtype=bool)
    new_face_list = []
...
def validate(candidate):
    """
    Fast watertight check — pack edge pairs into int64 for fast unique counting.
    A mesh is watertight iff every edge is shared by exactly 2 faces.
    """
    ...
    # Pack two vertex indices into one int64 — much faster than lexsort
